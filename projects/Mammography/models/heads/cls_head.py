...
class BinaryClsHead(BaseModule):
    """Classification head.
    ...
    def __init__(self,
        ...
        super(BinaryClsHead, self).__init__(init_cfg=init_cfg)

        self.final_active = "Sigmoid" if loss["use_sigmoid"] else "Softmax"
        print("final activation :" , self.final_active)

        if not isinstance(loss, nn.Module):
            loss = MODELS.build(loss)
        self.loss_module = loss
        self.cal_auc = cal_auc
    ...
    def _get_loss(self, cls_score: torch.Tensor,
        ...
        if self.cal_auc:
            if self.final_active == "Softmax":
                cls_score = F.softmax(cls_score, dim=1)
            else:
                cls_score = torch.sigmoid(cls_score)

            roc, pr = AUC.calculate(cls_score,target)
            losses.update({'roc_auc':roc,'pr_auc':pr})
    ...
    def _get_predictions(self, cls_score, data_samples):
        ...
        if self.final_active == "Softmax":
            ...
            pred_labels =  (F.softmax(cls_score, dim=1)).argmax(dim=1, keepdim=True).detach()
        else:
            pred_scores = cls_score
            pred_labels = torch.round(torch.sigmoid(cls_score))
            # pred_labels = cls_score
