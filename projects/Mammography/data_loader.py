...
def load_clinical_data(data_root,
    ...
    if mode == 'clinical-only':
        ...
        X = {split: np.load(f"{data_root}/{split}_x_postope.npy")
             for split in splits}
        print("X shape:", X['train'].shape)
    else:
        print("load clinical and mammo features")
        X={split: np.concatenate([np.load(f"{data_root}/{split}_x_postope.npy", allow_pickle=True),np.load(f"{data_root}/{split}_feat_5.npy",allow_pickle=True),
                                  ],axis=1)
        for split in splits}
        print("X shape:", X['train'].shape)
    #TODO softmax
...
def load_clinical_data_test(data_root,
    ...
    if mode == 'clinical-only':
        ...
        print("load clinical and mammo features")
        # X={split: np.concatenate([np.load(f"{data_root}/{split}_x_w_LVI.npy"),np.load(f"{data_root}/{split}_feat_5.npy")],axis=1)
